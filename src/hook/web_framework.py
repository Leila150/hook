...
import asyncio, inspect, json, re

class Request:
    def __init__(self,method,path,headers=None,body=b""):
        self.method=method.upper();self.path=path;parsed=urlparse(path);self.url=path;self.query=parsed.query;self.path_only=parsed.path;self.headers=dict(headers or {});self.body=body
    @property
...
class Response:
    def __init__(self,body="",status=200,headers=None):
        if isinstance(body,(dict,list,tuple)):body=json.dumps(body,ensure_ascii=False).encode("utf-8");base={"Content-Type":"application/json; charset=utf-8"}
    ...
    def match(self,method,path):
        ...
        for m,pattern,handler in self.routes:
            ...
            for hit in re.finditer(r"\{(\w+)(?::\s*([^}]+))?\}",pattern):
                parts.append(re.escape(pattern[pos:hit.start()]));names.append((hit.group(1),hit.group(2)));parts.append(r"([^/]+)");pos=hit.end()
            parts.append(re.escape(pattern[pos:]));hit=re.fullmatch("".join(parts),path)
            if not hit:continue
            converted=[]
            try:
                for (_,typ),value in zip(names,hit.groups()):
...
class WebApp:
    ...
    async def _invoke_async(self,fn,req,params):
        value=fn(req,**params);return await value if inspect.isawaitable(value) else value
    ...
    async def dispatch_async(self,req):
        ...
        async def call(index,request):
            ...
            result=mw(request,lambda r=request:call(index+1,r));return await result if inspect.isawaitable(result) else result
    ...
    def dispatch(self,req):
        ...
        def call(index,request):
            ...
            if signature is not None:
                ...
                result=mw(lambda r=request:call(index+1,r)) if len(positional)<2 else mw(request,lambda r=request:call(index+1,r))
    ...
    def serve(self,host="127.0.0.1",port=8000):
        ...
        class Handler(BaseHTTPRequestHandler):
            def do_any(self):
                try:length=int(self.headers.get("Content-Length","0"));req=Request(self.command,self.path,self.headers,self.rfile.read(length));res=app.dispatch(req)
