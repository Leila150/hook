...
try:
    ...
    try:
        ...
        raise ModuleNotFoundError("HOOK on Python 3.10 requires tomli; install hook-lang with its test/runtime dependencies") from exc
...
def _version(binary):
    try:
        p=subprocess.run([binary,"--version"],capture_output=True,text=True,timeout=5);return (p.stdout or p.stderr).splitlines()[0] if p.returncode==0 else ""
...
def find_tool(*names):
    ...
    return None
class Project:
    def __init__(self,root="."):self.root=Path(root).resolve();self.config={}
    def load(self):
        p=self.root/"hook.toml";self.config=tomllib.loads(p.read_text(encoding="utf-8")) if p.exists() else {};return self.config
    def validate(self):self.load();return self.root.exists() and (not self.config or self.config.get("project",{}).get("name",self.root.name))
    def write(self,name=None,version="0.1.0",entry="main.hk"):
        self.root.mkdir(parents=True,exist_ok=True);n=name or self.root.name;(self.root/"hook.toml").write_text(f'[project]\nname = "{n}"\nversion = "{version}"\nentry = "{entry}"\n\n[build]\ntarget = "host"\n',encoding="utf-8");return self.root/"hook.toml"
class AndroidToolchain:
    def __init__(self,android_sdk=None):
        raw=android_sdk or os.environ.get("ANDROID_SDK_ROOT") or os.environ.get("ANDROID_HOME");self.android_sdk=Path(raw).resolve() if raw else None
    def target(self,abi="arm64"):return f"android-{abi}"
    def available(self):return bool(self.android_sdk and self.android_sdk.exists() and (self.android_sdk/"platform-tools").exists())
    ...
    def build(self,root,package="app",name="HookApp"):
        ...
        subprocess.run([gradle,"assembleDebug"],cwd=project,check=True);candidates=list((project/"app/build/outputs/apk/debug").glob("*.apk"))
...
class Codegen:
    ...
    def compiler(self):return find_tool("clang","gcc") if self.target.startswith("windows") else find_tool("clang","gcc","cc")
    ...
    def compile_c(self,source,output,shared=False,extra_args=()):
        ...
        cmd += ["-x","c","-","-o",str(output)];p=subprocess.run(cmd,input=source,text=True,capture_output=True)
