"""AST, semantic analysis, and compiler IR for HOOK."""
...
class ASTParser:
    ...
    def __init__(self, source: str):
        self.source = source; self.lines = []; self.pos = 0
    ...
    def _block(self, indent: int):
        ...
        while self.pos < len(self.lines):
            ...
            if level > indent: raise SyntaxError(f"unexpected indentation at line {line}")
            self.pos += 1; node = self._node(text, line)
            if self.pos < len(self.lines) and self.lines[self.pos][1] > indent:
                node.children = self._block(self.lines[self.pos][1])
            out.append(node)
        return out

    def _node(self, text, line):
        patterns = [
            ...
            (r"^(?:import|from)\b", "import"),
            (r"^return\b", "return"),
            (r"^break(?:\s+\d+)?$", "break"),
            (r"^raise\b", "raise"),
        ]
        for pattern, kind in patterns:
            if re.search(pattern, text): return ASTNode(kind, text, line=line, column=1)
        return ASTNode("statement", text, line=line, column=1)

class SemanticError(Exception): pass

class SemanticAnalyzer:
    def __init__(self): self.scopes = [set()]; self.errors = []
    def analyze(self, tree): self._walk(tree); return self.errors
    def _walk(self, node):
        if node.kind == "declaration":
            ...
            elif m and not str(node.value).startswith("reassign"): self.scopes[-1].add(m.group(1))
...
@dataclass(frozen=True)
class Instruction:
    opcode: str; operands: tuple[Any, ...] = (); line: int = 0
...
class BytecodeCompiler:
    def compile(self, tree):
        p = BytecodeProgram(); self._compile_nodes(tree.children, p); p.instructions.append(Instruction("HALT")); return p
    def _compile_nodes(self, nodes: Iterable[ASTNode], p):
        for n in nodes:
            if n.kind == "return": p.instructions.append(Instruction("RETURN", (str(n.value)[6:].strip(),), n.line))
            elif n.kind == "break":
                parts = str(n.value).split(); p.instructions.append(Instruction("BREAK", (int(parts[1]) if len(parts)>1 else 1,), n.line))
            elif n.kind == "function": p.instructions.append(Instruction("DEFINE_FUNCTION", (str(n.value),), n.line))
            elif n.kind == "class": p.instructions.append(Instruction("DEFINE_CLASS", (str(n.value),), n.line))
            else: p.instructions.append(Instruction("EXEC_SOURCE", (str(n.value),), n.line))
            if n.children: self._compile_nodes(n.children, p)

class LanguagePipeline:
    def compile(self, source):
        tree = ASTParser(source).parse(); errors = SemanticAnalyzer().analyze(tree)
