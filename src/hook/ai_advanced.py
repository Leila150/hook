"""Dependency-free tensor, autograd, transformer and checkpoint primitives for HOOK."""
...
class Tensor:
    def __init__(self,data,requires_grad=False):
        self.data=data if isinstance(data,list) else float(data); self.requires_grad=requires_grad; self.grad=None
    @property
    ...
    def _binary(self,other,fn):
        a=_flat(self.data);b=_flat(other.data if isinstance(other,Tensor) else other); sa=self.shape; sb=other.shape if isinstance(other,Tensor) else ()
        if len(b)==1: b=b*len(a)
        if len(a)!=len(b):raise ValueError("incompatible tensor shapes")
        return Tensor(_build([fn(x,y) for x,y in zip(a,b)],list(sa)),self.requires_grad or isinstance(other,Tensor) and other.requires_grad)
...
class Module:
    ...
    def load_state_dict(self,state):
        for k,v in state.items():
            ...
            if hasattr(obj,parts[-1]):getattr(obj,parts[-1]).data=v
...
class MultiHeadAttention(Module):
    def __init__(self,dim,heads=1,seed=0):
        if dim%heads:raise ValueError("embedding dimension must divide number of heads")
    ...
    def __call__(self,x):
        rows=x.data; q=self.q(x).data;k=self.k(x).data;v=self.v(x).data;d=self.dim//self.heads;out=[]
        for i in range(len(rows)):
            scores=[sum(q[i][h*d:(h+1)*d][z]*k[j][h*d:(h+1)*d][z] for z in range(d))/math.sqrt(d) for j in range(len(rows)) for h in [0]]
            m=max(scores); ex=[math.exp(s-m) for s in scores];s=sum(ex);w=[e/s for e in ex]
            out.append([sum(w[j]*v[j][z] for j in range(len(rows))) for z in range(self.dim)])
...
class TinyTransformer(Module):
    ...
    def load(cls,path,**kwargs):
        obj=cls(**kwargs);obj.load_state_dict(json.loads(Path(path).read_text(encoding="utf-8")));return obj
...
class CharTokenizer:
    ...
    def encode(self,text):return [self.to_id[c] for c in text]
