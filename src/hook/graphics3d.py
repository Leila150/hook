"""HOOK 3D software renderer plus optional GPU capability detection."""
...
@dataclass
class Camera:
    position:Vec3=Vec3(0,0,-5); fov:float=70; near:float=.1; far:float=1000
    def project(self,p,width,height):
        q=p-self.position
        if q.z<=self.near:return None
...
class SoftwareRenderer:
    ...
    def render(self,mesh,position=Vec3(0,0,0)):
        zbuf=[[float("inf")]*self.width for _ in range(self.height)];pix=[[0]*self.width for _ in range(self.height)]
        ...
        for face in mesh.faces:
            ...
            minx=max(0,int(min(p[0] for p in tri)));maxx=min(self.width-1,int(max(p[0] for p in tri)));miny=max(0,int(min(p[1] for p in tri)));maxy=min(self.height-1,int(max(p[1] for p in tri)))
            ax,ay=tri[0][0],tri[0][1];bx,by=tri[1][0],tri[1][1];cx,cy=tri[2][0],tri[2][1];den=(by-cy)*(ax-cx)+(cx-bx)*(ay-cy)
            if den==0:continue
            for y in range(miny,maxy+1):
                for x in range(minx,maxx+1):
                    ...
                    if u>=0 and v>=0 and w>=0:
                        ...
                        if depth<zbuf[y][x]:zbuf[y][x]=depth;pix[y][x]=255
        return pix
...
class GPUBackend:
    ...
    def info(self):
        ...
        try:r=subprocess.run([self.backend,"--version"],capture_output=True,text=True,timeout=3);detail=(r.stdout or r.stderr).splitlines()[0] if r.returncode==0 else ""
        except Exception:detail=""
        return {"available":True,"backend":self.backend,"version":detail}
