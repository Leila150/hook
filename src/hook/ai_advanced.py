...
class Tensor:
    ...
    def _binary(self,other,fn):
        b=other.data if isinstance(other,Tensor) else other;shape=_broadcast_shape(self.shape,_shape(b));aa=_flat(_broadcast_to(self.data,shape));bb=_flat(_broadcast_to(b,shape));return Tensor(_build([fn(x,y) for x,y in zip(aa,bb)],list(shape)),self.requires_grad or (isinstance(other,Tensor) and other.requires_grad))
...
class Module:
    ...
    def load_state_dict(self,state):
        for k,v in state.items():
            obj=self;parts=k.split('.')
            for p in parts[:-1]:obj=obj[int(p)] if isinstance(obj,list) else getattr(obj,p)
            target=obj[int(parts[-1])] if isinstance(obj,list) else getattr(obj,parts[-1]);target.data=v
class Linear(Module):
    ...
    def __call__(self,x):
        ...
        if not a or not isinstance(a[0],list):return Tensor([sum(w*v for w,v in zip(row,a))+b for row,b in zip(self.weight.data,self.bias.data)])
        return Tensor([[sum(w*v for w,v in zip(row,values))+b for row,b in zip(self.weight.data,self.bias.data)] for values in a])
    ...
    def __call__(self,x):
        ...
        for h in range(self.heads):
            ...
            for i in range(len(rows)):
                scores=[sum(q[i][start+z]*k[j][start+z] for z in range(d))/math.sqrt(d) for j in range(len(rows))];m=max(scores);ex=[math.exp(s-m) for s in scores];total=sum(ex);weights=[e/total for e in ex]
