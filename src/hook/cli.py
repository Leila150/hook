...
def repl()->int:
    banner(); info("Interactive mode. Type :help for commands, :quit to exit."); e=Engine()
    while True:
        try:s=input("hook> ")
        except (EOFError,KeyboardInterrupt): print(); return 0
        c=s.strip().lower()
        if c in {":quit",":q",":exit"}: return 0
        if c in {":help",":h"}: print(":help  show this help\n:quit  exit HOOK\n:version  show version\n:clear  clear the terminal"); continue
...
def build_parser():
    p=argparse.ArgumentParser(prog="hook",description="HOOK programming language — simple, powerful, extensible.")
...
def main(argv=None)->int:
    ...
    if ns.target=="run":
        if not ns.file:error("run requires a .hk file"); return 2
        return _run_file(pathlib.Path(ns.file))
    if ns.target is None:p.print_help(); return 0
