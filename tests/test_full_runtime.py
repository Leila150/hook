...
from hook.full_runtime import (
    ...
    SourceMap, Profiler, Debugger, LSPServer, ProjectManifest, Serializer,
)
...
def test_neural_layers_and_optimizers():
    ...
    p.grad = 1
    old = p.value
    SGDOptimizer(model.parameters, lr=.1).step()
    assert p.value != old
    p.grad = 1
    AdamOptimizer(model.parameters).step()
...
def test_game_database_and_tooling(tmp_path):
    ...
    assert db.execute("select x from t")[0]["x"] == 4
    db.close()
...
def test_debug_lsp_and_sourcemap():
    sm = SourceMap(); sm.add(3, "main.hk", 9, 2)
    assert sm.resolve(3).source_line == 9
...
def test_native_compiler_if_available(tmp_path):
    ...
    out = tmp_path / ("libhook.so")
    compile_c("int hook_add(int a,int b){return a+b;}\n", str(out), shared=True)
    assert out.exists()


def test_android_project_generation(tmp_path):
    root = AndroidProject(tmp_path / "app", "com.example.hook")
    root.write()
