...
def ray_aabb(ray,box,max_distance=float('inf')):
    ...
    p=ray.at(tmin); eps=1e-6
    n=Vec3(-1,0,0) if abs(p.x-box.minimum.x)<eps else Vec3(1,0,0) if abs(p.x-box.maximum.x)<eps else Vec3(0,-1,0) if abs(p.y-box.minimum.y)<eps else Vec3(0,1,0) if abs(p.y-box.maximum.y)<eps else Vec3(0,0,-1) if abs(p.z-box.minimum.z)<eps else Vec3(0,0,1)
    return Hit3D(tmin,p,n)
...
def ray_triangle(ray,a,b,c,max_distance=float('inf')):
    ...
    t=inv*e2.dot(q)
    return None if t<0 or t>max_distance else Hit3D(t,ray.at(t),e1.cross(e2).normalized())
...
def _decode_gltf_buffer(doc,base,index):
    b=doc['buffers'][index]
    if 'uri' not in b:return b''
    uri=b['uri']
    if uri.startswith('data:'):return base64.b64decode(uri.split(',',1)[1])
    return (Path(base)/uri).read_bytes()

def load_gltf(path,material=None):
    path=Path(path); doc=json.loads(path.read_text(encoding='utf-8')); blobs=[_decode_gltf_buffer(doc,path.parent,i) for i in range(len(doc.get('buffers',[])))]
    meshes=[]
    for gm in doc.get('meshes',[]):
        for prim in gm.get('primitives',[]):
            attrs=prim.get('attributes',{}); accp=doc['accessors'][attrs['POSITION']]; bv=doc['bufferViews'][accp['bufferView']]; raw=blobs[bv['buffer']]; off=bv.get('byteOffset',0)+accp.get('byteOffset',0); stride=bv.get('byteStride',12); verts=[Vec3(*struct.unpack_from('<3f',raw,off+i*stride)) for i in range(accp['count'])]
            tris=[]
            if 'indices' in prim:
                ai=doc['accessors'][prim['indices']]; bi=doc['bufferViews'][ai['bufferView']]; raw=blobs[bi['buffer']]; typ={5121:'B',5123:'H',5125:'I'}[ai['componentType']; size=struct.calcsize('<'+typ); off=bi.get('byteOffset',0)+ai.get('byteOffset',0); ids=[struct.unpack_from('<'+typ,raw,off+i*size)[0] for i in range(ai['count'])]; tris=[tuple(ids[i:i+3]) for i in range(0,len(ids)-2,3)]
            else: tris=[(i,i+1,i+2) for i in range(0,len(verts)-2,3)]
            meshes.append(Mesh3D(verts,tris,material=material))
    return meshes
...
class AssetLoader:
    ...
    def mesh(path,material=None):
        ...
        if suffix=='.gltf':
            meshes=load_gltf(path,material); return meshes[0] if meshes else Mesh3D([],[])
        if suffix=='.glb': raise ValueError('GLB is not supported yet; use .gltf or extract its JSON/buffer payload')
        raise ValueError(f'unsupported 3D mesh format: {suffix or "<none>"}; supported: .obj, .gltf')
    @staticmethod
    def meshes(path,material=None):
        if Path(path).suffix.lower()=='.gltf':return load_gltf(path,material)
        return [AssetLoader.mesh(path,material)]
    @staticmethod
    def texture(path):
        suffix=Path(path).suffix.lower()
        ...
        try:
            ...
            if suffix in {'.png','.jpg','.jpeg','.bmp','.webp'}:
                im=Image.open(path).convert('RGBA'); pix=[Color(r/255,g/255,b/255,a/255) for r,g,b,a in im.getdata()]; return Texture2D(im.width,im.height,pix)
        except ImportError:pass
        raise ValueError(f'unsupported texture format: {suffix}; use .ppm or install Pillow for common image formats')
