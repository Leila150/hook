...
class ExtensionRegistry:
    ...
    def register(self, name, category, value=None, bases=(), replaces=None, **metadata):
        ...
        old = bucket.get(d.name)
        if old is not None and old.value is not value:
            self.conflicts.setdefault((category, d.name), [old]).append(d)
            return d
        bucket[d.name] = d
        self.active[category][replaces or d.name] = d
    ...
    def choose(self, name, category, extension_name):
        ...
        chosen = candidates.get(extension_name)
        if chosen is None: raise KeyError(extension_name)
        self.active[category][name] = chosen
        return chosen.value

    def replace(self, target, replacement, category):
        category = self._category(category)
        d = replacement if isinstance(replacement, ExtensionDefinition) else self.items[category].get(str(replacement))
        if d is None: raise KeyError(replacement)
        self.active[category][target] = d
        d.replaces = target
        return d.value
...
# Processor is intentionally the miscellaneous category. These are the
# built-in v0.1 syntax processors explicitly reserved by HOOK's design.
...
def install_extension_system(engine_cls):
    ...
    def builtins(self):
        ...
        # Extension parents are language values. A .hk class can therefore use
        # class Foo(Statement):, class X(Operator):, etc.
        for name in CATEGORIES:
            self.root.values[name] = HookType(name)
            self.types[name] = self.root.values[name]
    ...
    def exec_block(self, nodes, scope):
        ...
        for name, value in list(scope.values.items()):
            ...
            base_names = {getattr(b, "name", str(b)) for b in bases}
